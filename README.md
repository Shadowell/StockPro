...
> 当前交付边界：研究优先、仅模拟交易。日常开发在本地运行；配置好的生产环境通过 GitHub Actions 从 `main` 部署，但不开放真实券商下单能力。
...
当前操作台保持 BitPro 常驻侧栏，并以“策略 → 回测 → 模拟”为唯一执行主线。行情、股票池、
因子、盯盘、信号、监控、复盘、数据和 AI 研发均为研究与运行辅助页。实盘、期货、链上、
套利和 ARC 不注册产品路由。
...
| 信号 | `/signals` | 信号 payload、Paper lineage 与投递审计 |
