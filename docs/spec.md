...
- 回测复盘的 24H/7D/30D 窗口分别要求每策略至少 `2/5/20` 个交易日与权益点、`1/3/10`
  笔闭合交易，并要求数据水位完整。样本健康度以这四项为分母；任一门槛未满足时状态为
  `insufficient_sample`，评分为 null，不进入观察榜或复查榜，页面必须展示真实覆盖、采样点和诊断原因。
