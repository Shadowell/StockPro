...
产品仍保留完整工作区路由。2026-08-18 形态决策：侧栏露出核心链（数据/行情/策略/回测/模拟/AI 研发），
实验工作区（股票池/因子/监控/复盘）继续菜单隐藏；实盘 `/live` 留直链，不从菜单露出（直链路由均保留）：
...
菜单隐藏（路由保留）：股票池 `/pools`、因子 `/factors`、监控 `/monitor`、复盘 `/review`、实盘 `/live`、数据处理 `/data/processing`。
...
实盘工作台是受控入口：只展示券商通道状态、晋级候选、预检清单与审计事件；
未配置券商通道且未开启 `LIVE_TRADING_ENABLED` 时，所有部署请求被安全拦截并留痕。
...
- 实盘工作台（/live）只提供券商通道状态、晋级候选、预检清单、双重确认与审计留痕；真实委托需要券商量化通道（QMT/miniQMT 或 PTrade）配置并通过 `LIVE_TRADING_ENABLED` 显式开启，当前合同内不接入真实订单路由。
