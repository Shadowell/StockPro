...
- 新增本地数据库选择器，`start.sh` / `restart.sh` 只接受可达的
  `stockpro_bitpro_rebase_dev`，不读取服务器数据库地址、不建立 SSH 隧道；启动后同时核对后端、
  前端、存储健康与实际数据库名。
- 启动流程不再每次执行 pip/npm 安装；`stop.sh` 不再使用宽泛 `pkill`，只停止登记的 PID、tmux
  会话和 4444/4445 上可识别的 StockPro 进程，数据库与 Paper 历史不受停止/重启影响。
- 新增 `status.sh` 文本/JSON 状态，服务停止时仍可读取本地日线数量、标的数、日期范围与最近备份。
  新增 custom-format PostgreSQL 备份脚本，只有 `pg_restore --list` 与 SHA-256 校验完成后才发布备份。
- 本地隔离库先完成 23 MB 预变更备份，再应用 47→49 增量迁移并执行最新交易日物化；当前为
  682,753 条历史、5,547 条最新行情、4 个指数、9 个数据分区和 1 个 sealed 快照。最终 26 MB
  备份通过 SHA-256、`pg_restore --list`，并真实恢复到一次性数据库对账上述记录与 49 个迁移后删除。
- 10 项本地运行/隔离库回归完成 RED→GREEN；`scripts/check.sh` 通过 165 项后端、前端类型/lint/build/
  bundle/依赖审计、安全检查和 42 项 Mock E2E。真实 4444/4445 使用隔离库启动，首页、数据、行情
  页面无错误，首页显示 4 个真实指数与 5,547 只最新行情；本轮未访问服务器或触发远程部署。
...
## GitHub #45 数据中心同步任务明细容错（2026-08-27）

- 根因是 `/sync/jobs` 的兼容/历史记录可能缺少 `job_id`、数组或计数字段，但数据中心折叠态不会
  渲染任务行；管理员展开明细后才执行 `job.jobId.slice()`，触发页面级错误边界。
- 前端在 API 状态写入前统一归一化任务记录：非对象记录忽略，任务号和时间字段安全转字符串，
  symbols/timeframes 只保留有效字符串，计数只接受非负有限数字；缺失任务号显示 `—`，行键使用
  局部稳定回退，不再因单条坏记录摧毁整个 `/data` 页面。
- Playwright 回归直接注入缺少可选字段的任务，桌面与 390px 均连续展开成功；断言无 page error、
  无“页面加载失败”错误边界且窄屏无页面级横向溢出。同步行为、Provider 调用和数据库均未改变。
...
- 行情页默认切到 `1m`，开放分钟周期按钮，并在连接状态与 K 线证据条展示 AKShare/缓存/错误状态，
  避免把“没有缓存”显示成不明原因的空白。
- 证券搜索弹层不再默认截断 50 个结果；`/api/v2/market/symbols` 在仓库已有 TuShare 同步结果时
  展示全量库内股票，在仓库为空时只读拉取 AKShare `stock_zh_a_spot_em` 沪深京 A 股列表作为
  展示兜底，东财不可用时退到 AKShare `stock_info_a_code_name` 代码简称列表，不写库、不触发同步。

## 资金流实时分钟线代理（2026-08-27）

- `/api/v2/orderflow/bars` 已接入 TuShare `rt_min` 实时分钟线，支持 1/5/15/30/60 分钟频率，
  返回真实分钟 OHLCV、成交额、数据源和可用性状态；服务端按 TuShare 低频限制做 6 分钟缓存/
  退避，避免页面自动刷新持续打满额度。
...
- 新增回归测试覆盖 stream status、bars 转换、缓存复用、large-trades tick 边界和前端分钟线提示。
...
## 2026-08-28 首页市场驾驶舱物化

- 历史同步扩展为每个开放交易日同时读取 TuShare `daily`、沪深 300 `index_daily` 和真实
  `stk_limit`，在单事务封存日线、价格限制历史、涨跌停/炸板/一字板/连板梯队、六阶段、
  行业/概念成员快照、5/10/20/60 日 RPS 与异动指标；所有结果绑定同一个市场证据快照。
- 行业成员使用 `stock_basic.industry`；概念成员主源使用当前生产 Token 已验证可访问的
  `ths_index/ths_member`，按配额串行拉取并过滤昨日表现、宽基和通道标签。AKShare 东方财富
  概念接口保留为回退，但本机与生产当前探针分别受代理和 `ConnectionError` 阻断，不作为成功证据。
- 完整只读 Provider 探针按 0.4 秒节流完成：过滤后 386 个概念全部成功，当前成员关系
  63,050 条，排除 9 个宽基/通道标签，失败 0，耗时 208.9 秒；探针未写 PostgreSQL。
- 首页改为唯一 `GET /api/v2/market/dashboard` 请求，5 秒单航班缓存只读 PostgreSQL 结果；响应
...
- 生产任务 #6 在修复后成功：124 个真实行情日期、944,893 条有效历史价格限制；最新情绪为
  78 涨停、4 跌停、21 炸板、封板率 78.79%、最高 6 板、二板宽度 16，阶段为“高潮”，
...
## 2026-08-28 A 股 ARC 自主研究空态与代理恢复

- 恢复 `HyperTradeClient` 的任务、进度、候选、证据和管理员签名审批代理；创建请求硬校验
  `600519.SH` 形式的 A 股统一代码和 `1D` 周期，Paper 预授权固定为 false。
- `/arc/config` 返回连接状态、缺失配置、恢复路径、支持资产类型、周期和 A 股制度边界；上游未配置时
  任务读取返回明确的零任务空态，创建返回 503，数字资产标的在触达上游前返回 422。
- 页面默认值改为 `600519.SH / 1D`，展示连接与零写入状态；桌面和 390px 下标题、表单、KPI 与空态
  完整可读，无横向溢出，未创建回测、候选或 Paper。

## 2026-08-28 回测成交、闭合交易与委托一致性

- 回测列表和详情从 PostgreSQL 分别读取 `backtest_trades` 成交数、metrics 闭合交易数与
  `backtest_orders` 委托数；最新权益和实际成交费用也由事实表聚合，不再使用互相矛盾的 metrics 零值。
- 单日或零闭合交易将收益、年化、回撤、Sharpe、Sortino、胜率和利润因子标记为样本不足；基准少于
  2 根同区间有效日线时不计算超额，研究诊断显示具体原因。
- A 股金额统一为 `¥`，成交流水移除杠杆/保证金列，证券代码统一为交易所后缀格式。目标最小链路
  在列表和详情均为成交 1、闭合 0、委托 0、费用 ¥29.7621、期末权益 ¥999970.2379。

## 2026-08-28 复盘样本健康度与不可判定分组

- 复盘健康度不再固定为 100%；24H/7D/30D 分别使用每策略 `2/5/20` 个交易日与权益点、
  `1/3/10` 笔闭合交易及数据水位完整四项分母，返回实际覆盖、采样点、缺失率和诊断。
- 不足门槛的策略评分、收益、回撤、胜率与盈亏比均为 null，判决为“样本不足/不可判定”，只进入
  独立样本不足组，不参与观察榜、复查榜或组合指标。
- 生产目标样本应显示覆盖 2026-08-26 至 2026-08-26、权益点 1、成交 1、闭合 0；24H/7D/30D
  健康度分别为 50%/35%/27.5%，并说明 1H 热力图无法由日线权益证据生成。

## 2026-08-28 策略版本、逻辑与 Paper 关联证据

- 策略目录按 `strategy_version_id` 只读关联最新回测、Paper、持仓标的、容量约束和最近成交原因；
  关联 Paper 为 running/paused 时策略状态同步，实例控制台按 Paper 实例 ID 跳转。
- 最小链路详情展示 v1/content hash/stockpro.v1 valid、3 只 sealed 北交所样例池、日收益降序入场、
  未实现退出与调仓、单标的最大仓位 10%、A 股平台约束、回测 #2118348412 和 Paper #1452658566。
- 英文 minimal sample 描述与成交原因替换为中文审计说明，页面标记“样例 / 非投资建议”；代码和参数
  与同一策略版本一并展示，不把该样例冒充正式候选。

## 2026-08-28 A 股持仓名称展示
