...
Data：已交付隔离扩展数据暂存、CSV/JSON/XLSX 双向交换和默认关闭的 HTTPS 白名单连接器；公式、大小、主机、DNS 与重定向边界在写库/请求前验证，暂存记录不自动映射核心数据或 Paper。下一增量进入首次配置与复盘收口；ETF范围依赖仍缺 ETF 快照、Universe、成本与 Paper 语义，不伪装为已支持。
