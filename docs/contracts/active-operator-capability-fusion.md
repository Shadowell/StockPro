...
Review/Onboarding：已交付首页只读首次就绪检查，并从首页和模拟页统一进入既有 `/review` 盘后复盘；检查按安全、存储、Provider、封存数据、策略、Paper、复盘分别显示证据与 Owner 路由，不执行修复。功能阶段全部完成，进入全路由、权限、响应式、生产状态与主线连续性总验收；ETF 因缺独立快照、Universe、成本与 Paper 语义明确保留为未支持。
