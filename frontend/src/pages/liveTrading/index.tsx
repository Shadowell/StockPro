...
function buildTradingInstances(
  paperInstances: any[],
  strategies: StrategyInfo[],
): TradingInstance[] {
  const out: TradingInstance[] = [];

  for (const p of paperInstances) {
    const pid = String(p.instanceId ?? p.instance_id ?? p.id ?? '');
    if (!pid) continue;
    const status = normalizeInstanceStatus(p);
    if (!ACTIVE_INSTANCE_STATUSES.has(status)) continue;
    const cfg = asRecord(p.config);
    out.push({
      id: `paper:${pid}`,
      kind: 'paper',
      dryRun: true,
      ...
      assetClass: inferAssetClass({
        name: p.strategyName || p.strategy_name,
        symbol: p.symbol,
        symbols: p.symbols,
        config: cfg,
      ...
      name: String(p.strategyName || p.strategy_name || '模拟实例'),
      symbol: String(p.symbol || '—'),
      timeframe: String(p.timeframe || '—'),
      status,
      createdAt: optionalText(p.createdAt ?? p.created_at),
      strategyType: optionalText(cfg.strategy_type ?? cfg.strategyType),
      strategyKey: optionalText(cfg.strategy_key ?? cfg.strategyKey),
      isAiAutonomous: isAiAutonomousStrategy({
        name: String(p.strategyName || p.strategy_name || ''),
        config: cfg,
      }),
      capitalVersion: finiteNumber(cfg.initial_capital ?? p.initialCapital ?? p.initial_capital, NaN),
      leverage: resolveInstanceLeverage(cfg),
      totalPnl: finiteNumber(p.totalPnl ?? p.total_pnl ?? p.pnl ?? p.equity?.change, 0),
      totalReturnPct: Number(p.totalReturnPct ?? p.total_return_pct ?? 0),
      winRate: Number(p.winRate ?? p.win_rate ?? 0),
      profitFactor: Number(p.profitFactor ?? p.profit_factor ?? 0),
      sharpeRatio: Number(p.sharpeRatio ?? p.sharpe_ratio ?? 0),
      maxDrawdownPct: Number(p.maxDrawdownPct ?? p.max_drawdown_pct ?? 0),
      stopKind: 'paper',
    });
  }
...
function LiveTradingWorkspace({ modeScope }: { modeScope?: TradeMode }) {
  ...
  const rawInstances = useMemo(
    () => buildTradingInstances(paperInstances, strategies),
    [paperInstances, strategies],
  );
  ...
  useEffect(() => {
    ...
    const refreshDashboardLists = async () => {
      ...
      try {
        await Promise.all([loadStrategies(), loadPaperInstances()]);
      } finally {
      ...
      const entries = await Promise.all(
        active.map(async (inst) => {
          try {
            ...
            if (inst.id.startsWith('paper:')) {
              const pk = paperInstanceKey(inst.id);
              if (!pk) return [inst.id, {}] as const;
              const raw = await paperApi.getInstance(pk);
              const p = raw?.data !== undefined ? raw.data : raw;
              return [
                inst.id,
                {
                  totalPnl: finiteNumber(
                    p?.totalPnl ?? p?.total_pnl ?? p?.pnl ?? p?.equity?.change,
                    inst.totalPnl ?? 0,
                  ),
                  totalReturnPct: finiteNumber(
                    p?.totalReturnPct ?? p?.total_return_pct,
                    inst.totalReturnPct ?? 0,
                  ),
                  winRate: finiteNumber(p?.winRate ?? p?.win_rate, inst.winRate ?? 0),
                  profitFactor: finiteNumber(p?.profitFactor ?? p?.profit_factor, inst.profitFactor ?? 0),
                  sharpeRatio: finiteNumber(p?.sharpeRatio ?? p?.sharpe_ratio, inst.sharpeRatio ?? 0),
                  maxDrawdownPct: finiteNumber(
                    p?.maxDrawdownPct ?? p?.max_drawdown_pct,
                    inst.maxDrawdownPct ?? 0,
                  ),
                },
              ] as const;
            }
  ...
  useEffect(() => {
    ...
    if (pk) {
      ...
      const loadPaper = async () => {
        try {
          const r = await paperApi.getInstance(pk);
          const data = r?.data !== undefined ? r.data : r;
          if (!cancelled) setPaperDetail(data);
        } catch {
          if (!cancelled) setPaperDetail(null);
        }
      };
      loadPaper();
      const timer = setInterval(loadPaper, 10000);
      return () => {
        cancelled = true;
        clearInterval(timer);
      };
  ...
  const handleDeletePaper = async (instanceId: string) => {
    ...
    try {
      ...
      loadPaperInstances();
      if (activeInstanceId === `paper:${instanceId}`) {
        setActiveInstanceId(null);
        setView('dashboard');
      }
    } catch (err) {
