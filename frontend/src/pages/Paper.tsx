...
const displaySymbol = (value: unknown) => {
  const symbol = String(value || '')
  const match = symbol.match(/^(SH|SZ|BJ)_(\d{6})$/)
...
export default function Paper() {
  ...
  const load = async (silent = false) => {
    ...
    try { const [instances, backtests] = await Promise.all([paperCurrentApi.list('audit'), backtestCurrentApi.runs(200)]); setItems(instances.items); setRuns(backtests.items); setError('') }
