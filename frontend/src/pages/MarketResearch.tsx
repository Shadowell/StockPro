...
function Calendar({ items }: { items: MarketCalendarEvent[] }) {
  return <section className={`${panel} overflow-hidden`}><div className="border-b border-crypto-border px-5 py-4"><h2 className="font-semibold text-white">交易事件日历</h2><p className="mt-1 text-xs text-slate-500">本地事件缓存；无记录不代表当日无事件。</p></div><div className="divide-y divide-white/[0.04]">{items.slice(0, 80).map((item, index) => { const row = item as unknown as Record<string, unknown>; return <div key={index} className="grid gap-2 px-5 py-4 sm:grid-cols-[130px_120px_1fr]"><span className="tabular-nums text-sm text-blue-300">{String(row.event_date ?? row.date ?? '--')}</span><span className="text-xs text-slate-500">{String(row.event_type ?? row.type ?? '市场事件')}</span><span className="text-sm text-slate-200">{String(row.title ?? row.name ?? row.content ?? '--')}</span></div>; })}{items.length === 0 ? <div className="p-12 text-center text-sm text-slate-600">事件缓存为空或尚未同步；不能据此判断“无事件”。</div> : null}</div></section>;
}
