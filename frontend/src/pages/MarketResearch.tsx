...
export function MarketResearch() {
  ...
  return <div className="min-h-full bg-crypto-bg px-5 py-6 2xl:px-8" data-testid="market-research-workbench" data-operator-page="market">
    ...
    {!context && loading && tab !== 'calendar' && tab !== 'stock' && tab !== 'watchlist' && tab !== 'indices' ? <div className={`${panel} flex h-72 items-center justify-center text-slate-500`}><RefreshCw className="mr-2 h-5 w-5 animate-spin" />读取市场快照…</div> : null}
    {!loading && !context?.snapshot && tab !== 'calendar' && tab !== 'stock' && tab !== 'watchlist' && tab !== 'indices' ? (
    ...
    {tab === 'indices' ? <MarketIndicesPanel /> : null}
