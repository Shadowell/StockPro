...
export default function FactorLab() {
  ...
  return <div className="h-full overflow-y-auto bg-crypto-bg p-4 sm:p-6">
    ...
    {loading && !summary ? <div className="mt-6 flex min-h-64 items-center justify-center rounded-xl border border-crypto-border bg-crypto-card text-sm text-gray-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />正在读取 FactorLab…</div> : <>
      <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-6"><MetricCard label="定义总数" value={statistics?.definitionCount || 0} note="注册定义" tone="cyan" /><MetricCard label="参数实例" value={statistics?.instanceCount || 0} note="有效不可变版本" tone="blue" /><MetricCard label="研究任务" value={statistics?.researchTaskCount || 0} note="全部状态" tone="amber" /><MetricCard label="Trial" value={statistics?.trialCount || 0} note="失败也保留" tone="slate" /><MetricCard label="最新值" value={statistics?.latestValueCount || 0} note="已物化样例" tone="amber" /><MetricCard label="物化分区" value={statistics?.materializedPartitionCount || 0} note="sealed snapshot" tone="slate" /></div>
