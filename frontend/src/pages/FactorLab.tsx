    ...
    {loading && !summary ? <div className="mt-6 flex min-h-64 items-center justify-center rounded-xl border border-crypto-border bg-crypto-card text-sm text-gray-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />正在读取 FactorLab…</div> : <>
      ...
      <section className={`mt-4 grid 2xl:grid-cols-[440px_minmax(0,1fr)] ${panelClass}`}>
        <div className="border-b border-crypto-border p-4 2xl:border-b-0 2xl:border-r"><div className="flex items-center gap-2"><BrainCircuit className="h-4 w-4 text-cyan-300" /><h2 className="text-sm font-medium text-white">因子实验配置</h2></div>
          ...
          <button type="button" disabled className="mt-3 inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-cyan-500/35 bg-cyan-500/10 text-sm font-medium text-cyan-100 disabled:opacity-40"><Play className="h-4 w-4" />启动研究待接通</button><div className="mt-2 text-center text-[10px] text-amber-300/80">当前只读真实因子定义和封存快照，不创建任务或 Paper。</div>
        </div>
      ...
      <div className="mt-4 grid gap-4 2xl:grid-cols-[minmax(0,1fr)_340px]"><section className={`min-w-0 ${panelClass}`}><div className="flex items-center justify-between border-b border-crypto-border px-4 py-3"><div className="flex items-center gap-2"><Boxes className="h-4 w-4 text-cyan-300" /><h2 className="text-sm font-medium text-white">因子定义</h2></div><span className="text-xs text-gray-500">连续值 · 阈值不写入定义</span></div><div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-xs"><thead className="bg-gray-950/55 text-gray-500"><tr><th className="px-4 py-2.5">因子 / 版本</th><th className="px-4 py-2.5">家族 / 角色</th><th className="px-4 py-2.5">默认参数</th><th className="px-4 py-2.5">预热</th><th className="px-4 py-2.5">方向语义</th><th className="px-4 py-2.5">实现</th></tr></thead><tbody className="divide-y divide-crypto-border">{summary?.definitions.length ? summary.definitions.map((definition) => <tr key={`${definition.definitionId}@${definition.definitionVersion}`} className="hover:bg-white/[0.02]"><td className="px-4 py-3 align-top"><div className="font-medium text-gray-100">{definition.displayName}</div><div className="mt-1 font-mono text-[11px] text-gray-500">{definition.definitionId}@{definition.definitionVersion}</div><div className="mt-1 max-w-xs text-[11px] leading-4 text-gray-600">{definition.description}</div></td><td className="px-4 py-3 align-top text-gray-300"><div>{familyLabels[definition.family] || definition.family}</div><div className="mt-1 text-gray-500">{roleLabels[definition.role] || definition.role}</div></td><td className="px-4 py-3 align-top font-mono text-[11px] text-blue-200">{defaultParameters(definition)}</td><td className="px-4 py-3 align-top text-gray-300">{definition.lookbackBars} 根<div className="mt-1 text-gray-500">实例 {instancesByDefinition.get(definition.definitionId) || 0}</div></td><td className="px-4 py-3 align-top text-gray-400">{orientationLabels[definition.orientation] || definition.orientation}</td><td className="px-4 py-3 align-top"><div className="font-mono text-[11px] text-gray-300">{definition.kernelName}</div><div className="mt-1 font-mono text-[10px] text-gray-600">{shortHash(definition.implementationHash)}</div></td></tr>) : <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-500">暂无已注册因子定义</td></tr>}</tbody></table></div></section><aside className={panelClass}><div className="flex items-center gap-2 border-b border-crypto-border px-4 py-3"><ShieldCheck className="h-4 w-4 text-amber-300" /><h2 className="text-sm font-medium text-white">数据与运行边界</h2></div><BoundaryRow ok label="26 个连续因子可用" detail="默认内置二十六个；目录读取全部已注册。成交量包含 Z-Score、MFI、量价相关性、OBV 与 VWAP；新增短线、流动性、涨停次数、跳空和日内收益因子。" /><BoundaryRow ok={Boolean(summary?.capabilities.materializationStoreReady)} label="真实数据集快照" detail="下一根 open 标签，20bps / 40bps 成本，Parquet 与 manifest 可复查。" /><BoundaryRow ok={Boolean(summary?.capabilities.researchMetricsAvailable)} label="机器学习验证可用" detail="等权、Ridge、Logistic；至少 5 个 OOS fold，分数不能覆盖硬门槛。" /><BoundaryRow ok={false} label="尚未接入策略运行时" detail="研究结果不会改变原策略信号、仓位或退出保护。" /><BoundaryRow ok={false} label="未连接 Paper / Live" detail="不读取账户，不创建模拟盘，不发送订单。" /><div className="m-4 rounded-xl border border-blue-500/20 bg-blue-500/[0.06] p-3 text-xs leading-5 text-blue-200/80">ADX14 ≥ 18、MFI ≥ 80 等条件属于实验组合；因子定义仍保存连续值。</div></aside></div>
      <section className={`mt-4 ${panelClass}`}><div className="flex flex-wrap items-center justify-between gap-2 border-b border-crypto-border px-4 py-3"><div className="flex items-center gap-2"><FileStack className="h-4 w-4 text-blue-300" /><h2 className="text-sm font-medium text-white">已注册参数实例</h2></div><div className="flex items-center gap-3 text-xs text-gray-500"><span className="inline-flex items-center gap-1"><Database className="h-3.5 w-3.5" /> SQLite 控制面</span><span className="inline-flex items-center gap-1"><Cpu className="h-3.5 w-3.5" /> ML Worker</span><span className="inline-flex items-center gap-1"><Gauge className="h-3.5 w-3.5" /> 严格预热</span></div></div><div className="overflow-x-auto"><table className="w-full min-w-[880px] text-left text-xs"><thead className="bg-gray-950/55 text-gray-500"><tr><th className="px-4 py-2.5">实例 ID</th><th className="px-4 py-2.5">定义</th><th className="px-4 py-2.5">参数</th><th className="px-4 py-2.5">所需 K 线</th><th className="px-4 py-2.5">状态</th></tr></thead><tbody className="divide-y divide-crypto-border">{summary?.instances.length ? summary.instances.map((instance) => <tr key={instance.instanceId} className="hover:bg-white/[0.02]"><td className="px-4 py-3 font-mono text-[11px] text-gray-300">{instance.instanceId}</td><td className="px-4 py-3 text-gray-400">{instance.definitionId}@{instance.definitionVersion}</td><td className="px-4 py-3 font-mono text-[11px] text-blue-200">{instance.parametersJson}</td><td className="px-4 py-3 tabular-nums text-gray-300">{instance.requiredBars} 根</td><td className="px-4 py-3"><span className="rounded-lg border border-cyan-500/25 bg-cyan-500/10 px-2 py-1 text-[11px] text-cyan-200">{instance.isDefault ? '默认实例' : '已注册实例'}</span></td></tr>) : <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-500">暂无参数实例</td></tr>}</tbody></table></div></section>
    </>}
  </div>;
}
