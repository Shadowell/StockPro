...
function toStartMs(value: string) { return Date.parse(`${value}T00:00:00Z`); }
function toEndMs(value: string) { return Date.parse(`${value}T23:59:59.999Z`); }
...
export default function FactorLab() {
  ...
  const [symbols, setSymbols] = useState('BTC/USDT:USDT\nETH/USDT:USDT');
  ...
  const startResearch = async () => {
    const normalizedSymbols = symbols.split(/[\n,]/).map((item) => item.trim()).filter(Boolean);
    if (!selectedFactors.length || !normalizedSymbols.length) { setError('至少选择一个因子和一个标的'); return; }
    if (!Number.isFinite(toStartMs(startDate)) || !Number.isFinite(toEndMs(endDate)) || toEndMs(endDate) <= toStartMs(startDate)) { setError('研究日期范围无效'); return; }
    if (mode !== 'manual' && (!selectedProvider?.configured || !model)) { setError('自动组合需要已配置并验证的 Provider'); return; }
    const manualCombinations = mode === 'auto' ? [] : [{ hypothesis: '操作者固定权重组合', expression: { type: 'weighted_sum', terms: selectedFactors.map((instanceId) => ({ weight: weights[instanceId] ?? 1, node: { type: 'factor', instance_id: instanceId } })) } }];
    setSubmitting(true); setError('');
    try {
      const created = await factorLabApi.createResearchTask({ exchange: 'okx', marketType, symbols: normalizedSymbols, timeframe, startMs: toStartMs(startDate), endMs: toEndMs(endDate), mode, factorInstanceIds: selectedFactors, manualCombinations, providerKey: mode === 'manual' ? '' : providerKey, model: mode === 'manual' ? '' : model, reasoningEffort: mode === 'manual' ? 'auto' : reasoningEffort, speedMode: mode === 'manual' ? 'standard' : speedMode, horizonBars, baseCostBps: 20, stressCostBps: 40, minCoverage: 0.95, nSplits: 5, maxCandidates, maxRuntimeSec: maxRuntimeMinutes * 60, maxNoImprovement, maxCombinationLeaves: Math.min(8, selectedFactors.length), targetAcceptedCandidates: 1, randomSeed: 42 });
      setTasks((current) => [created, ...current.filter((item) => item.taskId !== created.taskId)]); setSelectedTaskId(created.taskId); setTrials([]);
    } catch (caught) { setError(parseApiError(caught, '启动因子研究失败')); }
    ...
    {loading && !summary ? <div className="mt-6 flex min-h-64 items-center justify-center border border-crypto-border bg-crypto-card text-sm text-gray-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />正在读取 FactorLab…</div> : <>
      ...
      <section className="mt-4 grid border border-crypto-border bg-crypto-card 2xl:grid-cols-[440px_minmax(0,1fr)]">
        <div className="border-b border-crypto-border p-4 2xl:border-b-0 2xl:border-r"><div className="flex items-center gap-2"><BrainCircuit className="h-4 w-4 text-cyan-300" /><h2 className="text-sm font-medium text-white">因子实验配置</h2></div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2"><Field label="市场"><select className={inputClass} value={marketType} onChange={(event) => setMarketType(event.target.value as 'spot' | 'swap')}><option value="swap">合约</option><option value="spot">现货</option></select></Field><Field label="K 线周期"><select className={inputClass} value={timeframe} onChange={(event) => setTimeframe(event.target.value)}><option value="15m">15M</option><option value="1h">1H</option><option value="4h">4H</option><option value="1d">1D</option></select></Field><Field label="开始日期"><input className={inputClass} type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></Field><Field label="结束日期"><input className={inputClass} type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} /></Field></div>
          ...
          <button type="button" onClick={() => void startResearch()} disabled={submitting || !selectedFactors.length} className="mt-3 inline-flex h-9 w-full items-center justify-center gap-2 border border-cyan-500/35 bg-cyan-500/10 text-sm font-medium text-cyan-100 hover:bg-cyan-500/15 disabled:opacity-40">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}启动研究</button><div className="mt-2 text-center text-[10px] text-amber-300/80">结果只形成研究候选，不会自动创建 Paper，也不会连接 Live。</div>
        </div>
