...
const strategy = { id: 'strategy-1', name: 'A股动量', version: 1, validation_status: 'valid', description: '封存股票池动量策略', content_hash: 'hash-1', script_content: 'def initialize(context):\n    pass\n\ndef handle_data(context, data):\n    pass', data_dependencies: ['daily_bars'] }
...
test('strategy backtest paper is the only execution mainline', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/backtest\?strategy_version_id=strategy-1/)
