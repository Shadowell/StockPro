...
test('monitor separates lifecycle from health evidence', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '模拟盘总览' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '监控配置' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '模拟盘监控' })).toBeVisible()
  const row = page.locator('[data-paper-instance-id="paper-1"]')
  await expect(row).toContainText('running')
  await expect(row).toContainText('stale')
  await expect(page.getByText('生命周期与健康分离', { exact: true })).toBeVisible()
  await expect(page.getByText(/USDT|交易所连接|真实账户/)).toHaveCount(0)
  await expect(page.getByRole('heading', { name: '实盘总览' })).toHaveCount(0)
})
