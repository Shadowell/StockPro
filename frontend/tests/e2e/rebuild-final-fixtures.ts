...
export async function installFinalFixtures(page: Page, role: 'admin' | 'guest' = 'admin') {
  await page.route('**/api/v2/**', async (route: Route) => {
    ...
    else if (path === '/api/v2/market/timeline') data = { items: [{ trade_date: '2026-08-27', phase: '高潮', phase_status: 'ok', confidence: 0.8, reasons: ['上涨占比 72.0%'], phase_missing_inputs: [], sentiment_status: 'ok', limit_up_count: 78, limit_down_count: 4, failed_limit_count: 21, seal_rate_pct: 78.8, highest_streak: 6, ladder_width: 16, source_snapshot_id: 7, snapshot_consistent: true }], data_status: 'ok', limit: 60, writes_performed: false, paper_mutated: false }
    ...
    else if (path === '/api/v2/factorlab/summary') data = { status: 'ready', phase: 'a_share_factor_research', statistics: { definition_count: 20, instance_count: 1, latest_value_count: 3, materialized_partition_count: 1, research_task_count: 0, trial_count: 0 }, definitions: [{ definition_id: '20', definition_version: 1, display_name: '真实收盘收益', family: 'momentum', role: 'alpha', description: '封存日线收益', kernel_name: 'minimal_real_close_return_1d', inputs: ['stock_history'], parameter_schema: {}, lookback_bars: 2, availability: 'EOD', orientation: 'higher', missing_policy: 'preserve_null', implementation_hash: 'hash', status: 'materialized', metadata: { data_status: 'ok', input_row_count: 3, latest_trade_date: '2026-08-26', latest_value_count: 3 } }], instances: [{ instance_id: 'fv:1', definition_id: '20', definition_version: 1, parameters_json: '{}', parameters: {}, parameter_hash: 'hash', required_bars: 2, created_at: '2026-08-26T18:00:00+08:00', is_default: true }], latest_values: [], data_plane: { format: 'PostgreSQL', layout: 'factor_versions + factor_daily_values', manifest: 'sealed-hash' }, capabilities: { api_mode: 'read_write_research_ledger', materialization_store_ready: true, research_metrics_available: true, strategy_runtime_connected: false, paper_live_connected: false } }
    ...
    else if (path === '/api/v2/onchain/summary') data = { status: 'partial', symbol: '600519.SH', name: '贵州茅台', board: '主板', industry: '白酒', as_of: '2026-08-28T00:00:00+08:00', valuation: { trade_date: '2026-08-26', close: 1302.8, pe_ttm: 19.9991, pb: 6.4819, ps_ttm: 9.5715, dividend_yield_ttm: 3.9932, total_market_cap_cny: 1628606308480, float_market_cap_cny: 1628606308480, source: 'tushare.daily_basic', source_snapshot_id: 1, available_at: '2026-08-27T10:22:03+08:00', knowledge_cutoff_at: '2026-08-27T10:22:03+08:00' }, latest_factors: {}, items: [], missing_inputs: ['尚未同步公告时点财务/股东/分红事实'], provider_calls: 0, writes_performed: false, orders_created: 0, paper_mutated: false }
