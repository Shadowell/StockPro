...
'''
    ...
    def _upsert_paper_position(self, position: Dict[str, Any]) -> None:
        ...
        if self._is_postgres():
            ...
            cursor.execute(
                ...
                (account_id, strategy_id, symbol, name, quantity, avg_price,
                 last_price, market_value, pnl, pnl_pct, updated_at)
                VALUES ({ph}, {ph}, {ph}, {ph}, {ph}, {ph}, {ph}, {ph}, {ph}, {ph}, datetime('now'))
                """,
                params,
            )
        conn.commit()
    ...
    def _is_postgres(self) -> bool:
        return self.db.__class__.__name__.lower().startswith("postgres")

    def _placeholder(self) -> str:
        return "%s" if self._is_postgres() else "?"

    def _now_expr(self) -> str:
        return "CURRENT_TIMESTAMP" if self._is_postgres() else "datetime('now')"
