...
class AuthService:
    ...
    def get_session(self, token: str | None) -> dict[str, Any] | None:
        ...
        # Touch throttle: every authenticated request used to UPDATE
        # last_seen_at, competing with backtest/paper writers for the SQLite
        # write lock (prod review 2026-08-24: 120 "database is locked" in 48h).
        # A 60s granularity is plenty for a last-active timestamp.
        last_seen = _parse_dt(row["last_seen_at"])
        if _now() - last_seen >= self._LAST_SEEN_TOUCH_INTERVAL:
            self.db.get_connection().execute(
                "UPDATE auth_sessions SET last_seen_at = ? WHERE id = ?",
                (_iso(_now()), session["session_id"]),
            )
            self.db.get_connection().commit()
            session["last_seen_at"] = _iso(_now())
