...
from app.services.agent.providers.contracts import ProviderCapabilities, ProviderError
...
class AgentOrchestrator:
    ...
    def create_task(
        ...
        *,
        ...
        task = AgentTask(
            ...
            llm_reasoning_effort=req.llm_reasoning_effort,
            llm_speed_mode=req.llm_speed_mode,
            created_at=now,
            updated_at=now,
        )
        # Resolve and pin the Provider at task creation.  Later changes to the
        # global Provider selection must not reroute an existing task.
        task.pin_provider_config(provider_capabilities)
    ...
    async def run_task(self, task_id: str) -> AgentTask:
        ...
        try:
            ...
            if task.status == "running" and evaluated_iterations == 0:
                raise RuntimeError("任务没有完成任何可评估迭代")
            ...
            task.status = "failed"
            self._set_stage(task, "failed", "Provider 执行失败")
            task.updated_at = datetime.now().isoformat()
            await self._persist_task_state(task)
            raise
        except Exception as e:
