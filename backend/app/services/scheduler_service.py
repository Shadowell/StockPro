...
class SchedulerService:
    ...
    async def start(self):
        ...
        # 8b. 数据中心 OKX 原生数据同步：每分钟检查配置，rubik 资金流/多空比与
        #     OI 快照分别按配置间隔到期执行（默认 rubik 日频、OI 每小时）
        self._scheduler.add_job(
            self._okx_native_sync_job,
            IntervalTrigger(minutes=1),
            id='okx_native_sync',
            name='数据中心OKX原生数据同步',
        )
    ...
    async def _okx_native_sync_job(self):
        """按数据中心配置执行 OKX 原生数据同步（rubik 资金流/多空比 + OI 快照）。"""
        try:
            from app.services.okx_native_sync_service import okx_native_sync_service

            result = await okx_native_sync_service.run_due()
            skipped = result.get("skipped")
            if skipped in {"disabled", "not_due"}:
                logger.debug("[定时任务] OKX原生数据同步跳过: %s", skipped)
            else:
                logger.info("[定时任务] OKX原生数据同步完成: %s", result)
        except Exception as e:
            logger.error("[定时任务] OKX原生数据同步失败: %s", e)
