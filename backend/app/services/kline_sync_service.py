...
StockPro V1 不使用文件存储或 SQLite，把这些语义全部映射到 PostgreSQL。
"""
...
from app.db.local_db import db_instance as default_db
...
class KlineSyncService:
    ...
    def create_history_sync_job(
        ...
        if any(timeframe != "1d" for timeframe in normalized_timeframes):
            raise ValueError("StockPro V1 only supports A-share daily kline sync")
    ...
    def _fetch_from_akshare(self, symbol: str, timeframe: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        if self._normalize_timeframe(timeframe) != "1d":
            raise ValueError("Only daily kline sync is supported in V1")
    ...
    def _placeholder(self) -> str:
        return "%s" if self.db.__class__.__name__.lower().startswith("postgres") else "?"
