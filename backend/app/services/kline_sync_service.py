"""A 股 PG 日线同步服务（自 BitPro-first 重建底座恢复的运营能力）。

BitPro 的语义：K 线有同步元数据、同步任务和任务项。StockPro 使用单一
PostgreSQL 通道把这些语义映射到 ``sync_jobs`` / ``sync_job_items`` /
``kline_history``。

全市场日线优先按交易日拉取（TuShare ``daily(trade_date=...)``），避免
按标的逐一请求导致全市场 × 一年不可用。
"""
...
class KlineSyncService:
    ...
    def __init__(self, db=None, fetcher: Optional[KlineFetcher] = None):
        if db is None:
            raise ValueError("KlineSyncService 需要显式注入 PostgresDatabase 实例")
        self.db = db
    ...
    def create_history_sync_job(
        ...
        return self.db.create_sync_job(
            job_name=job_name or f"kline-sync-{pd.Timestamp.now().strftime('%Y%m%d%H%M%S')}",
    ...
    def create_market_daily_sync_job(
        ...
        name = job_name or f"market-daily-{pd.Timestamp.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def _load_symbol_name_map(self) -> Dict[str, str]:
        try:
            rows = self.db.get_all_stocks_realtime(include_listing_status=False) or []
        except Exception:
            return {}
        ...
        try:
            with self.db.get_connection() as connection:
                with connection.cursor() as cursor:
                    cursor.execute(
                        """
                        SELECT name FROM kline_history
                        WHERE symbol = ANY(%s) AND COALESCE(name, '') <> '' AND timeframe='1d'
