...
from app.services.agent.providers.managed_login import get_runtime_provider_capabilities
...
def _provider_snapshot_for_response(snapshot: Any) -> Dict[str, Any]:
    """Return only public capability metadata in task responses."""
...
def _mark_agent_task_provider_failed(task_id: str) -> None:
    try:
        persisted = db.update_agent_task_status(task_id, "failed", datetime.now().isoformat())
        if persisted is False or (isinstance(persisted, int) and persisted == 0):
            raise RuntimeError("status row not found")
    except Exception as exc:
        # Do not include DB paths, SQL or provider diagnostics in this log.
        logger.error(
            "Agent 任务 Provider 失败状态持久化失败: task=%s error_type=%s",
            str(task_id)[:96],
            exc.__class__.__name__,
        )
        raise ProviderError(
            "任务失败状态未能持久化，请稍后重试",
            error_code="provider_status_persist_failed",
            provider_key=None,
            status_code=503,
        ) from exc


def _mark_agent_task_provider_failed_or_http(task_id: str) -> None:
    try:
        _mark_agent_task_provider_failed(task_id)
...
async def auto_resume_interrupted_agent_tasks(updated_at: str, limit: int = 20) -> int:
    ...
    for db_task in db.get_interrupted_agent_tasks(updated_at=updated_at, limit=limit):
        ...
        try:
            ...
            logger.error("AI Lab 任务 %s Provider 恢复失败: %s", task_id, exc.error_code)
            try:
                _mark_agent_task_provider_failed(task_id)
            except ProviderError as persist_exc:
                logger.error("AI Lab 任务 %s 失败状态未持久化: %s", task_id, persist_exc.error_code)
        except Exception as e:
...
def _task_from_db(row: Dict[str, Any]) -> AgentTask:
    ...
    snapshot = row.get("llm_provider_snapshot")
    snapshot_is_empty = snapshot is None or snapshot == "" or snapshot == "{}" or snapshot == {}
    ...
    task = AgentTask(
        ...
        llm_reasoning_effort=str(row.get("llm_reasoning_effort") or "auto"),
        llm_speed_mode=str(row.get("llm_speed_mode") or "standard"),
        llm_provider_snapshot=snapshot,
        strategy_spec=StrategySpec.from_dict(spec) if isinstance(spec, dict) else None,
    )
    # Rows created before task-level Provider fields existed are pinned during
    # restoration and immediately written back.  A failed resolution leaves the
    # legacy row readable; the next execution will surface the typed Provider
    # error instead of silently switching to another route.
    if not task.llm_provider and snapshot_is_empty:
        try:
            task.pin_provider_config()
            _save_agent_task_state(task)
        except Exception as exc:
            logger.warning("Agent 任务 %s Provider 快照解析失败: %s", task_id, exc)
    else:
        # A task with a declared Provider must validate its persisted snapshot;
        # malformed or drifted rows fail closed and are never repinned globally.
...
async def create_task(req: CreateTaskRequest):
    try:
        provider_key = str(req.llm_provider or get_llm_model_config().get("provider_key") or "dashscope").strip()
        capabilities = await get_runtime_provider_capabilities(ProviderRegistry(), provider_key)
        task = orchestrator.create_task(req, provider_capabilities=capabilities)
    except ProviderError as exc:
    ...
    return {
        ...
        "llm_reasoning_effort": task.llm_reasoning_effort,
        "llm_speed_mode": task.llm_speed_mode,
        "llm_provider_snapshot": _provider_snapshot_for_response(task.llm_provider_snapshot),
...
async def resume_task(task_id: str):
    ...
    try:
        ...
        # The normal accessor deserializes historical JSON before returning.
        # If that legacy state is corrupt, update status through the narrow
        # SQL-only path and expose the same sanitized task-state error used by
        # the in-memory restore path.
        _mark_agent_task_provider_failed_or_http(task_id)
        raise HTTPException(
            status_code=409,
            detail={
                "code": "task_state_invalid",
                "error_code": "task_state_invalid",
                "message": "Agent 任务历史状态数据无效",
            },
        ) from exc
    if not db_task:
        raise HTTPException(404, f"任务 {task_id} 不存在")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"code": "task_resume_failed", "error_code": "task_resume_failed", "message": "任务恢复失败"},
        ) from exc
...
async def generate_strategy(req: GenerateStrategyRequest):
    ...
    try:
        # Resolve at call time so the legacy llm_client injection seam remains
        # available to callers and tests that patch the shared client factory.
        from app.services.agent import llm_client

        client = llm_client.get_qwen_client()
    except ProviderError as e:
        raise HTTPException(status_code=e.status_code, detail=e.detail) from e
...
7. 所有方法必须是 async def，不要用 time.sleep；运行时上下文由基类提供
8. 只允许导入 numpy/math/collections；禁止导入 asyncio、logging 等其他模块
...
请生成完整可运行的 BaseStrategy 子类代码。"""
    ...
    # Generation quality varies per model call; one repair round that feeds the
    # concrete failure back recovers most rejections without weakening the gates.
    from app.services.agent.code_sandbox import validate_base_strategy_contract

    result: Optional[Dict[str, Any]] = None
    class_name = file_name = description = code = ""
    last_error = ""
    for attempt in range(2):
        user_content = user_msg
        if attempt:
            user_content = (
                f"{user_msg}\n\n上一次生成未通过校验：{last_error}\n"
                "请严格修正后重新返回完整 JSON。"
            )
        try:
            result = await client.chat_json(
                ...
                temperature=0.5,
                # Reasoning models spend the token budget on reasoning first; a
                # 4096 cap returned empty JSON content with finish_reason=length.
                max_tokens=16384,
            )
        except Exception as e:
            last_error = f"LLM 调用失败: {e}"
            continue
        class_name = str(result.get("class_name") or "AutoGenStrategy")
        file_name = str(result.get("file_name") or "auto_gen_strategy")
        description = str(result.get("description") or req.prompt[:100])
        code = str(result.get("code") or "")
        if not code or "BaseStrategy" not in code:
            last_error = "生成的代码未包含 BaseStrategy 子类"
            continue
        try:
            validate_base_strategy_contract(code)
        except Exception as e:
            last_error = f"代码安全检查失败: {e}"
            continue
        break

    if result is None or not code:
        raise HTTPException(500, last_error or "LLM 生成失败")
...
def _db_task_to_status(task: dict) -> dict:
    ...
    return {
        ...
        "llm_provider": task.get("llm_provider") or "",
        "llm_model": task.get("llm_model") or "",
        "llm_reasoning_effort": task.get("llm_reasoning_effort") or "auto",
        "llm_speed_mode": task.get("llm_speed_mode") or "standard",
        "llm_provider_snapshot": _provider_snapshot_for_response(task.get("llm_provider_snapshot")),
