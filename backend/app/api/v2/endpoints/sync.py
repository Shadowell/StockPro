...
async def status(
    include_items: bool = Query(
        False,
        description="是否内嵌当前任务的逐交易对明细；轮询场景保持 false 以避免 2MB+ 响应",
    ),
):
    return ok(sync_domain_service.status(include_items=include_items))
...
async def okx_native_schedule_config():
    from app.services.okx_native_sync_service import okx_native_sync_service

    return ok(okx_native_sync_service.schedule_config())


@router.put("/okx-native/schedule")
async def update_okx_native_schedule_config(payload: Dict[str, Any] = Body(default_factory=dict)):
    from app.services.okx_native_sync_service import okx_native_sync_service

    return ok(okx_native_sync_service.update_schedule_config(payload))


@router.post("/okx-native/run")
async def run_okx_native_sync(payload: Dict[str, Any] = Body(default_factory=dict)):
    from app.services.okx_native_sync_service import okx_native_sync_service

    kind = str(payload.get("kind") or "all").lower()
    if kind in ("rubik", "all"):
        rubik = await okx_native_sync_service.run_rubik_sync()
    else:
        rubik = {"skipped": "not_requested"}
    if kind in ("oi", "all"):
        oi = await okx_native_sync_service.run_oi_snapshot()
    else:
        oi = {"skipped": "not_requested"}
    return ok({"rubik": rubik, "oi": oi})
