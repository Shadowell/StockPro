...
def validate_factor_expression(
    ...
    *,
    allowed_parameters: set[str] | frozenset[str] | None = None,
) -> None:
    """Reject arbitrary code, unknown fields/operators and future references."""
    _validate_node(
        expression,
        path="expression",
        allowed_parameters=None if allowed_parameters is None else frozenset(allowed_parameters),
    )
...
def _validate_node(
    ...
    *,
    ...
    if isinstance(node, str) and node.startswith("$") and len(node) > 1:
        if allowed_parameters is not None and node[1:] not in allowed_parameters:
            raise FactorExpressionError(f"{path}: unknown factor parameter: {node}")
        return
...
def _validate_parameter_reference(
    ...
    *,
    ...
    if isinstance(value, str) and value.startswith("$") and len(value) > 1:
        if allowed_parameters is not None and value[1:] not in allowed_parameters:
            raise FactorExpressionError(f"{path}: unknown factor parameter: {value}")
        return
