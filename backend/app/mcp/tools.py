...
def mcp_selfcheck(client: Any) -> dict[str, Any]:
    """Issue #705 observability: round-trip the internal API and report pool policy.

    Returns a structured diagnostic instead of raising, so agents can use it to
    tell "MCP transport is fine but internal HTTP hangs" apart from a healthy
    backend without digging through server logs.
    """
    started = time.monotonic()
    error: str | None = None
    try:
        client.request("GET", "/system/health", tool_name="mcp_selfcheck")
    ...
    return {
        "ok": error is None,
        "error": error,
        "round_trip_path": "/system/health",
        "latency_ms": latency_ms,
        "api_base": str(getattr(client, "base_url", "")),
        "http_pool": {
            "client_class": type(http_client).__name__ if http_client is not None else None,
            "request_timeout_sec": getattr(client, "timeout", None),
            "keep_alive_enabled": bool(getattr(client, "keep_alive", False)),
            "connect_retries": getattr(client, "connect_retries", None),
            "keepalive_expiry_sec": getattr(client, "keepalive_expiry_sec", None),
            "default_connection_header": (
                str(http_client.headers.get("connection", "")).lower()
