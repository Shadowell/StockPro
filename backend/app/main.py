...
async def lifespan(app: FastAPI):
    ...
    # 启动 OKX 大单逐笔流采集（订单流页面数据源）
    from app.services.okx_large_trade_stream_service import okx_large_trade_stream_service
    await okx_large_trade_stream_service.start()
    logger.info("OKX large trade stream started")
    ...
    # 预热实盘工作台策略解析缓存：/live/strategies 冷态需对全部 DB 脚本策略
    # 逐个 AST+exec+合同校验（生产实测 6.9-16s），不预热时部署后的第一波
    # 用户请求直接承担该成本。后台执行，不阻塞启动。
    async def _warm_live_workspace_strategy_cache() -> None:
        try:
            from app.api.v2.endpoints.live import _list_live_execution_strategies

            started = time.perf_counter()
            await asyncio.to_thread(_list_live_execution_strategies)
            logger.info(
                "Live workspace strategy cache warmed in %.2fs",
                time.perf_counter() - started,
            )
        except Exception as e:
            logger.warning("Live workspace strategy cache warmup skipped: %s", e)

    asyncio.create_task(_warm_live_workspace_strategy_cache())
    ...
    # 停止 OKX 大单逐笔流采集
    from app.services.okx_large_trade_stream_service import okx_large_trade_stream_service
    await okx_large_trade_stream_service.stop()
