...
class CtaTrendFollowingStrategy(ContractStrategyBase):
    ...
    CIRCUIT_BREAKER_STATE_KEY = "circuit_breaker"
    ...
    async def on_init(self) -> None:
        ...
        self.risk_manager = RiskManager(
            RiskConfig(
                ...
                circuit_breaker_reset_mode=str(cfg.get("circuit_breaker_reset_mode", "permanent")).strip().lower(),
            )
        )
        equity = self._account_equity()
        if equity > 0:
            self.risk_manager.initialize(equity)
        # 恢复跨重启的熔断状态（paper/live 重启不再静默绕过总回撤熔断）
    ...
    async def on_bar(self, bar: BarData) -> None:
        if not is_finite_price(bar.close):
            return
        self._latest_bar_ts_ms = int(bar.timestamp)
    ...
    def _restore_circuit_breaker_state(self) -> None:
        """从 runtime state 恢复跨重启的总回撤熔断状态。

        后端重启会重建策略实例并 initialize(当前权益)，若不恢复，
        熔断会被静默清零（部署一次绕过一次最后防线）。
        config.circuit_breaker_acknowledge=true 表示人工确认解除：
        跳过恢复并清除持久化状态。
        """
        raw = self._runtime_state_readonly().get(self.CIRCUIT_BREAKER_STATE_KEY)
        if not isinstance(raw, dict) or not raw.get("active"):
            return
        if bool(self.config.get("circuit_breaker_acknowledge")):
            runtime = self._runtime_state()
            runtime.pop(self.CIRCUIT_BREAKER_STATE_KEY, None)
            logger.warning("CTA circuit breaker manually acknowledged; cleared persisted state")
            return
        try:
            self.risk_manager.restore_circuit_state(raw)
            logger.warning("CTA circuit breaker restored from runtime state: %s", raw.get("reason"))
        except Exception:
            logger.exception("Restore CTA circuit breaker state failed")

    def _persist_circuit_breaker_state(self) -> None:
        """把熔断状态写入 runtime state（引擎每根 bar 后持久化到 DB）。"""
        try:
            runtime = self._runtime_state()
            snapshot = self.risk_manager.export_circuit_state()
            if snapshot.get("active"):
                runtime[self.CIRCUIT_BREAKER_STATE_KEY] = snapshot
            else:
                runtime.pop(self.CIRCUIT_BREAKER_STATE_KEY, None)
        except Exception:
            logger.exception("Persist CTA circuit breaker state failed")

    def backtest_diagnostics(self) -> Dict[str, Any]:
        """回测诊断（BTStrategyAdapter 在 stop() 时收集）：暴露熔断状态。"""
        try:
            return {"circuit_breaker": self.risk_manager.get_circuit_breaker_snapshot()}
        except Exception:
            return {}
    ...
    def _risk_sized_notional(self, symbol: str, side: str, price: float, volatility: float) -> float:
        ...
        if equity <= 0 or price <= 0 or volatility <= 0:
            return 0.0
        check = self.risk_manager.check_account_drawdown(equity, now_ms=getattr(self, "_latest_bar_ts_ms", None))
        # 触发/重置都发生在本次检查内，立即写入 runtime state（引擎每 bar 持久化）
