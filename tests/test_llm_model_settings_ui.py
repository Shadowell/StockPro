...
def test_settings_dialog_can_activate_llm_provider_from_ui():
    main_layout = MAIN_LAYOUT.read_text(encoding="utf-8")
    client = CLIENT.read_text(encoding="utf-8")
    card = (ROOT / "frontend" / "src" / "components" / "settings" / "LLMProviderCard.tsx").read_text(encoding="utf-8")
...
def test_provider_settings_use_capability_card_component():
    main_layout = MAIN_LAYOUT.read_text(encoding="utf-8")
    card = (ROOT / "frontend" / "src" / "components" / "settings" / "LLMProviderCard.tsx").read_text(encoding="utf-8")
    helper = (ROOT / "frontend" / "src" / "components" / "settings" / "providerState.ts").read_text(encoding="utf-8")

    assert "./settings/LLMProviderCard" in main_layout
    assert "mergeLLMProviderSettings" in main_layout
    assert "providerCapabilities" in helper
    assert "getLLMProviderCapabilities" in card
