...
#   ./scripts/check.sh
#
# Creates stockpro_bitpro_rebase_dev via docker compose (preferred) or SQL
# against an already-running Postgres. Does not touch production or BitPro.
...
Usage: ./scripts/setup_isolation_db.sh [--print-url] [--migrate] [--sql-only] [--help]
  ...
  --print-url   Print the isolation DATABASE_URL and exit (no Docker/SQL).
...
echo "[setup] target URL: $DEFAULT_URL"
...
if [ "$SQL_ONLY" -eq 0 ] && have_docker; then
  compose_up
  if docker compose --file "$COMPOSE_FILE" --profile isolation exec -T postgres \
    test -f /docker-entrypoint-initdb.d/zz-stockpro-isolation.sql; then
