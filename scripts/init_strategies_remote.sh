...
echo "[INFO] DB: /opt/stockpro/backend/.env DATABASE_URL"

ssh -o BatchMode=yes "$STOCKPRO_SEED_SSH" "mkdir -p /opt/stockpro/{strategies,scripts}"
...
ssh -o BatchMode=yes "$STOCKPRO_SEED_SSH" \
  "cd /opt/stockpro && /opt/stockpro/backend/venv/bin/python scripts/init_strategies.py $FORCE_FLAG"
