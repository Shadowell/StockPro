...
DEFAULT_ISOLATION_URL = "postgresql://stockpro:stockpro@127.0.0.1:55432/stockpro_bitpro_rebase_dev"
...
def _connect(url: str, autocommit: bool = False):
    try:
        ...
        raise SystemExit(
            "psycopg is required. Install backend requirements or run "
            "./scripts/setup_isolation_db.sh which uses docker exec / psql."
        ) from exc
    connection = psycopg.connect(url, autocommit=autocommit)
    return connection
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Maintenance Postgres URL (postgres or stockpro).",
    )
    ...
    if args.migrate:
        ...
        if applied:
            print("[provision] applied migrations:")
            for version in applied:
                print(f"  - {version}")
        else:
            print("[provision] no pending migrations")
    return 0
